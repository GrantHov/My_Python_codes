...
## it uses fasta format file from multiple alignment with no insertion compatred to rCRS
...
Remaining_CR = open("Remaining_CR.fasta", "w")
PC1_and_PC2 = open("PC1+PC2.fasta", "w")
PC3 = open("PC3.fasta", "w")
#Third_nucleotide = open("Third_nucleotide.fasta", "w")
HVS12 = open("HVS12.fasta", "w")
#HVS2 = open("HVS2.fasta", "w")
rtRNA = open("rtRNA.fasta", "w")
#rRNA = open("rRNA.fasta", "w")

for seq in file2.readlines()[1:]:
    if re.search(r">",seq):
            
                name = seq.strip()
                Remaining_CR.write("%s\n"%(name))
                #rRNA.write("%s\n"%(name))
                rtRNA.write("%s\n"%(name))
                PC1_and_PC2.write("%s\n"%(name))
                PC3.write("%s\n"%(name))
                ...
                ATP8 = seq[8365:8524]#Taken from Hoviks file # NO OVERLAP
                ATP6 = seq[8574:9204]#Taken from Hoviks file # NO OVERLAP
                #intergenic13 = seq[9204:9206]
                COX3 = seq[9209:9989] # NO OVERLAP
                ...
                ND4L = seq[10469:10757]#Taken from Hoviks file#NO OVERLAP
                ND4 = seq[10768:12136]#Taken from Hoviks file #NO OVERLAP
...
#               print All_genes
                ...
                for nucl in splited_All_genes:
                        ...
                        All_genes_1_and_2_nucl.append(both)
                        All_genes_3_nucl.append(third)

                   
                All_genes_1_and_2_nucl = ''.join(All_genes_1_and_2_nucl)
                ...
                PC3.write("%s\n"%(All_genes_3_nucl))
                PC1_and_PC2.write("%s\n"%(All_genes_1_and_2_nucl))
                rtRNA.write("%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s\n"%(sRNA12,sRNA16,tRNA1Phe,tRNA2Val,tRNA3Leu,tRNA4Ile,tRNA5Gln,tRNA6Meth,tRNA7Try,tRNA8Ala,tRNA9Asp,tRNA10Cys,tRNA11Tyr,tRNA12Ser,tRNA13Asp,tRNA14Lys,tRNA15Gly,tRNA16Arg,tRNA17His,tRNA18Ser,tRNA19Leu,tRNA20Glu,tRNA21Thre,tRNA22Pro))
                #HVS2.write("%s\n"%(HVSII))
                #rRNA.write("%s%s\n"%(sRNA12,sRNA16))
                Remaining_CR.write("%s\n"%(CR))
                HVS12.write("%s%s\n"%(HVSI,HVSII))
